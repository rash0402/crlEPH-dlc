...
function generate_spm_3ch(
    ...
    heading = 0.0
    if norm(ego_vel) > 0.001
        heading = atan(ego_vel[2], ego_vel[1])
    end
    ...
    for (idx, p_rel) in enumerate(agents_rel_pos)
        ...
        # Calculate angle relative to heading (Ego-centric)
        theta_global = atan(p_rel[2], p_rel[1])
        theta_val = theta_global - heading
        ...
        # FOV masking (210 degrees)
        if abs(theta_val) > params.fov_rad / 2
            continue
        end
