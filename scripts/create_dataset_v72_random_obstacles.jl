#!/usr/bin/env julia
"""
VAE Training Data Collection for v7.2: Random Obstacles Scenario (5D State Space)
...
Purpose:
  ...
  - Unstructured environment with randomly placed obstacles
  - Storage: ~25 MB per simulation (includes heading)

Configuration:
  - Scenario: Random Obstacles (4-group + circular obstacles)
  - World: 100m × 100m
  - Obstacles: 30-70 circular obstacles (2-4m radius)
  - D_max = 6.0m (12×12 SPM grid)
  - Safety threshold = 4.0m (collision avoidance)
  - Exploration noise = 0.3 std
  - Repulsion strength = 2.0
  - Densities: 10,15,20 agents/group (default)
  - Obstacle counts: 30,50,70 (default)
  - Seeds: 1,2,3 (default)

Output:
  - Raw trajectories: data/vae_training/raw_v72/v72_random_d{density}_n{n_obs}_s{seed}_YYYYMMDD_HHMMSS.h5
  - HDF5 structure: trajectory/{pos, vel, heading, u, d_goal, group}, events/{collision, near_collision}

Usage:
  julia --project=. scripts/create_dataset_v72_random_obstacles.jl --densities 10,15,20 --obstacle-counts 30,50,70 --seeds 1,2,3 --steps 1500
"""
...
function parse_commandline()
    s = ArgParseSettings(description="VAE Training Data Collection v7.2 (5D State Space - Random Obstacles)")
...
"""
Run single simulation with v7.2 random walk controller (Random Obstacles)
"""
function run_simulation_v72(
    ...
    filename = "v72_random_d$(density)_n$(num_obstacles)_s$(seed)_$(timestamp).h5"
    ...
    h5open(filepath, "w") do file
        ...
        metadata_group["scenario"] = "random_obstacles"
        metadata_group["version"] = "v7.2"
        metadata_group["density"] = density
        metadata_group["num_obstacles"] = num_obstacles
...
function main()
    ...
    obstacle_counts = parse.(Int, split(args["obstacle-counts"], ','))
    ...
    println("  Scenario: Random Obstacles (4-group + obstacles)")
    println("  World: 100m × 100m")
    ...
    for density in densities
        for num_obstacles in obstacle_counts
            for seed in seeds
                println("  Scenario=random_obstacles, Density=$density, Obstacles=$num_obstacles, Seed=$seed")
                collision_rate, near_collision_rate = run_simulation_v72(
                    density, num_obstacles, seed, max_steps, output_dir
                )
