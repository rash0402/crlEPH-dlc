...
function main()
    println("=" ^ 60)
    println("🔬 EXPLORATION DATA COLLECTION")
    println("=" ^ 60)
    ...
    vae_model = nothing
    println("\n🧠 VAE: Disabled (pure data collection mode)")
    
    # Select agent for detailed logging
    detail_agent_id = 1
    
    println("\n" * "=" ^ 60)
    println("▶️  Starting exploration simulation...")
    println("=" ^ 60)
    ...
    for step in 1:world_params.max_steps
        ...
        for agent in agents
            # Compute ego-centric frame rotation
                ...
                rot_angle = π/2  # Default: pointing north
                ...
                for other in agents
                    if other.id != agent.id
                        ...
                        push!(local_agents, Float64[r_rel_ego[1], r_rel_ego[2], Float64(Int(other.group))])
                    end
                end
                ...
                if agent.id == detail_agent_id
                    ...
                    precision = 1.0  # Neutral precision during data collection
        ...
        # Progress logging
        if step % 500 == 0
            @printf("Step %4d/%d | Exploration mode: ε=%.1f%%, σ=%.1f%%\\n",
    ...
    println("\n" * "=" ^ 60)
    println("✅ Exploration data collection complete!")
    ...
    println("\n🎉 Done! Ready for VAE training with diverse data.")
end
