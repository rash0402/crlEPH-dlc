#!/usr/bin/env julia
"""
VAE Training Data Collection for v7.2: Corridor Scenario (5D State Space)
...
Purpose:
  ...
  - Direction-based goals: d_goal ∈ {E, W} (left-right movement)
...
Output:
  - Raw trajectories: data/vae_training/raw_v72/v72_corridor_d{density}_s{seed}_YYYYMMDD_HHMMSS.h5
  - HDF5 structure: trajectory/{pos, vel, heading, u, d_goal, group}, events/{collision, near_collision}

Usage:
  julia --project=. scripts/create_dataset_v72_corridor.jl --densities 10,15,20 --seeds 1,2,3 --steps 1500
"""
...
function parse_commandline()
    s = ArgParseSettings(description="VAE Training Data Collection v7.2 (5D State Space - Corridor)")
...
"""
Run single simulation with v7.2 random walk controller (Corridor)
"""
function run_simulation_v72(
    ...
    filename = "v72_corridor_d$(density)_s$(seed)_$(timestamp).h5"
    ...
    h5open(filepath, "w") do file
        ...
        metadata_group["scenario"] = "corridor"
        ...
        metadata_group["corridor_width"] = corridor_width
        metadata_group["corridor_length"] = corridor_length
...
function main()
    ...
    corridor_length = args["corridor-length"]
    ...
    println("  Scenario: Corridor (2-group bidirectional)")
    println("  Corridor: $(corridor_width)m × $(corridor_length)m")
    ...
    for density in densities
        for seed in seeds
            println("  Scenario=corridor, Density=$density, Seed=$seed")
            collision_rate, near_collision_rate = run_simulation_v72(
                density, seed, max_steps, corridor_width, corridor_length, output_dir
            )
