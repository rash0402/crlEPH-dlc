...
公共空間や社会的環境における自律システムの実運用では、安全性と社会的受容性を両立しつつ、環境の不確実性に柔軟に適応する能力が不可欠である。

しかし、混雑環境や多主体が相互作用する状況では、他者行動や環境変化の予測困難性が本質的に高くなり、従来のモデル予測制御（MPC）や強化学習（RL）に基づく手法は、過度に保守的な行動生成や、不安定な振動、さらには行動の停止といった **不確実性起因の行動破綻** を引き起こしやすい。

これらの問題は、不確実性を単に抑制すべきノイズや誤差として扱い、行動生成を調停する **能動的な設計変数として組み込めていない** ことに起因する。

すなわち、環境や他者の振る舞いに対する確信度の違いが、知覚や意思決定の「鋭さ」にどのように反映されるべきかという設計原理が、工学的に明示されていない点に本質的な課題がある。

### 目的 (Objective)
本研究の目的は、自由エネルギー原理（Free Energy Principle; FEP）および Active Inference の枠組みに基づき、不確実性を **Haze（推論的不確実性の操作的プロキシ）** として定量化し、それを **知覚表現の解像度（Precision）制御** に結び付けることで、不確実性に起因する多様な行動破綻を構造的に抑制する **不確実性適応型行動生成原理** を確立することである。

本研究は、混雑環境における移動ロボットの安定化を一つの具体例としつつも、その射程を単体ロボットに限定せず、群知能や分散型自律システムにおいても成立する、より一般的な設計原理として提示することを目指す。

### 学術的新規性 (Academic Novelty)

本研究の学術的新規性は、自由エネルギー原理における Precision 制御を、単なる予測誤差項の重み付けとしてではなく、**自己中心的知覚表現（SPM: Saliency Polar Map）における適応的な知覚解像度設計**として工学的に実装した点にある。

提案手法 EPH では、不確実性が高い状況において知覚表現をあえて平均化し、意思決定を鈍化させる一方、確信度が高い状況では知覚を鋭敏化し、効率的かつ目的指向的な行動を可能とする。

このような **知覚の幾何構造そのものを不確実性に応じて変形する設計** は、固定 Precision に基づく既存の MPC や RL、ならびに固定温度パラメータを用いる SOTA 手法とは本質的に異なるアプローチであり、Active Inference の工学的拡張として明確な差異（Delta）を有する。

### 手法 (Methods)

提案手法 **EPH (Emergent Perceptual Haze)** は、以下の 5 段階から構成される。

(i) 観測情報の自己中心的 Saliency Polar Map（SPM）への変換  
(ii) 変分オートエンコーダ（VAE）に基づく世界モデルによる将来状態予測  
(iii) 予測分散に基づく Haze（推論的不確実性）の推定  
(iv) Haze に基づく Precision の導出と、それに応じた SPM soft 集約パラメータ $\beta(H)$ の連続変調  
(v) 自由エネルギー最小化に基づく加速度入力の生成  

この構成により、不確実性は行動生成を抑制すべき外乱ではなく、**知覚と行動の関係を調停する中核的な制御信号**として機能する。

### 検証目標 (Validation Goals)

検証では、固定 Precision に基づくベースライン手法（Fixed-Precision MPC）を比較対象とし、不確実性の高い状況における行動破綻の抑制効果を多面的に評価する。

単体ロボットの混雑ナビゲーションを対象とする場合には、**立ち往生（Freezing）発生率の低減** や **運動の滑らかさ（Jerk）の改善** を主要指標とする。

さらに、群知能・分散系への拡張においては、**通過流量（Throughput）**、**局所密度の時間積分（渋滞指標）**、**群分断率**、および **振動指標** を用い、不確実性適応型知覚設計が集団挙動に与える影響を検証する。

### 結論と意義 (Conclusion / Academic Significance)

本研究は、不確実性を抑制すべき対象としてではなく、知覚と行動を調停するための **設計可能な情報源** として再定義する点に学術的意義がある。

EPH により、Active Inference の理論的枠組みは、単体ロボットの社会ナビゲーションに留まらず、群知能や分散型自律システムにおける行動破綻の理解と制御へと拡張される。

本成果は、ロボット工学における安全で柔軟な行動生成の設計原理を提供するのみならず、認知科学における計算論的柔軟性の理解や、不確実性下での AI 安全性設計にも理論的基盤を与えるものである。

**Keywords**: Free Energy Principle, Active Inference, Emergent Perceptual Haze, Perceptual Precision, Swarm Intelligence, Uncertainty-Adaptive Control
...
3. **実証**: 不確実性起因の行動破綻（例：Freezing、振動、分断）を対象として、適応的 Precision が破綻モードを構造的に抑制することを単体ロボットおよび群知能で実証する。
...
本研究の Haze → Precision → $\beta$ 変調は、**メタ適応制御（meta-adaptive control）** の一形態とみなせる。すなわち、制御器のパラメータを直接調整するのではなく、知覚表現の解像度を制御することで、間接的に行動生成の保守性・積極性を連続的に調節する。
これは、不確実性下での explorative / exploitative 行動バランスを、**認知的注意（attention）の配分問題** として再定式化したものである。

**課題**:
Haze に基づく $\beta$ 変調が、閉ループ系全体の漸近安定性や有界性を保証するための十分条件は未確立である。特に、$\beta$ が急激に変化する場合のゲイン切り替え（switched system）としての解析が必要となる。また、複数エージェント環境における分散 Haze 推定と、グローバル安定性の関係も今後の課題である。
...
## 3.X 群知能への拡張（Swarm Intelligence Extension）

### 3.X.1 位置づけ
EPH は単体ロボットの社会ナビゲーションに留まらず、
**局所的不確実性が集団全体の挙動を支配する分散系（群知能）**へ自然に拡張可能である。
本研究では、Freezing を代表例とする「行動破綻」を、
群知能における **渋滞・分断・振動** と同型の現象として捉える。

### 3.X.2 局所 Haze の定義（MVP 実装）
各エージェント i は近傍集合 N_i[k] に基づき、
簡易な運動予測誤差から局所 Haze を推定する。

$$
\hat{\boldsymbol{x}}_j[k] = \boldsymbol{x}_j[k-1] + \boldsymbol{v}_j[k-1] \Delta t
$$

$$
H_i[k] = \mathrm{Agg}(
\{ \| \boldsymbol{x}_j[k] - \hat{\boldsymbol{x}}_j[k] \|^2 \}_{j \in N_i[k]})
$$

### 3.X.3 Precision による群行動の鋭さ変調
$$
\Pi_i[k] = \frac{1}{H_i[k] + \epsilon}
$$

Precision は BOIDs 行動における **注意の鋭さ（softmax 温度）** に写像される。

$$
\beta_i[k] = \beta^{min} + (\beta^{max} - \beta^{min}) s(\Pi_i[k])
$$

### 3.X.4 Soft Attention 化された分離行動
...
$$
\boldsymbol{a}_{sep,i}[k] =
\sum_j \alpha_{ij}[k] \frac{-\boldsymbol{r}_{ij}[k]}{\|\boldsymbol{r}_{ij}[k]\| + \epsilon}
$$
...
3. **群知能・分散系指標（拡張）**: Throughput（狭隘通過流量）、局所密度の時間積分（渋滞指標）、群分断率（connected components）、速度・加速度の高周波成分（振動）。

### 4.3 アブレーションスタディ：固定 β vs Haze 適応 β(H)

> **仮説 H**:
> 
> soft 集約の鋭さ（$\beta_r, \beta_\nu$）を Haze / Precision に応じて適応的に変調することで、混雑環境における行動破綻（例：Freezing、振動、分断）が抑制され、安全性と社会的滑らかさが同時に向上する。
...
- **S1**: 通常 BOIDs（固定係数、固定鋭さ）
- **S2**: Soft Attention のみ（$\beta$ 固定）
- **S3**: $\beta(H)$ 変調のみ（attention 無し）
- **S4**: **EPH-Swarm**（Haze → Precision → $\beta$ 変調 + Soft Attention）

### 4.4 本章の要点

- 検証は仮説駆動型に設計されており、固定 $\beta$ との比較で因果連鎖を直接検証する。
- Freezing 抑制と社会的滑らかさに加え、群知能における渋滞・分断・振動を評価することで、分散系一般への妥当性を担保する。
...
- **応用**: Freezing を含む不確実性起因の行動破綻を設計原理レベルで抑制。
